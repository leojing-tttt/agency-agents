...
# runbooks (no agent frontmatter), not agents. apps/ is product code (e.g.
# campaign-agent), not a specialist roster folder. None of these is a division
# — they must never be scanned as source-agent categories, and must not be
# registered as fake divisions in divisions.json.
NON_DIVISION_DIRS=(examples scripts integrations strategy apps)
